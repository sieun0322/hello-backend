...
server.registerTool(
  ...
  async ({ title, lang }) => {
    ...
    if (!res.ok) {
      return {
        content: [{ type: "text", text: `문서를 찾을 수 없습니다: ${title}` }],
      };
    }
...
server.registerTool(
  ...
  async ({ title, lang }) => {
    ...
    if (!page.extract) {
      return {
        content: [{ type: "text", text: `본문을 찾을 수 없습니다: ${title}` }],
      };
    }

    // 너무 길면 3000자로 제한
    const extract =
      page.extract.length > 3000
...
server.registerTool(
  ...
  {
    description:
      "Wikipedia 문서를 가져온 후, MCP 서버가 Claude에게 한 줄 요약을 역으로 요청합니다 (Sampling 데모)",
    inputSchema: {
      title: z.string().describe("Wikipedia 문서 제목"),
      lang: z.enum(["ko", "en"]).default("ko").describe("언어 (ko 또는 en)"),
    },
  ...
  async ({ title, lang }) => {
    ...
    // Step 2: Sampling - MCP 서버 → Claude에게 요약 요청 (역방향)
...
// Resource: wiki://{lang}/{title} URI로 Wikipedia 문서를 리소스로 노출
// Tools와 차이: Claude가 "읽는" 데이터 소스 (실행하는 함수가 아님)
server.registerResource(
  ...
  async (uri: URL, variables: Record<string, string | string[]>) => {
    ...
    if (!res.ok) {
      return {
        contents: [{ uri: uri.href, text: `문서를 찾을 수 없습니다: ${title}`, mimeType: "text/plain" }],
      };
    }

    const data = (await res.json()) as { title: string; extract: string };
    return {
      contents: [{ uri: uri.href, text: `# ${data.title}\n\n${data.extract}`, mimeType: "text/plain" }],
    };
  }
);
...
server.registerPrompt(
  "explain-simple",
  {
    ...
    argsSchema: {
      topic: z.string().describe("설명할 주제"),
    },
  },
  ({ topic }) => ({
    messages: [
      {
        ...
        content: {
          type: "text",
          text: `Wikipedia에서 "${topic}"을 찾아서 초등학생도 이해할 수 있게 쉽게 설명해줘. 어려운 용어는 풀어서 설명하고, 실생활 예시를 들어줘.`,
        },
      },
    ],
  })
    ...
    messages: [
      {
        ...
        content: {
          type: "text",
          text: `Wikipedia에서 "${topic1}"과 "${topic2}"를 각각 찾아서 공통점과 차이점을 표로 정리해줘.`,
        },
      },
    ],
  })
